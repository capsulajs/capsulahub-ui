...
var couple = function couple(sectors, sector) {
  return sectors.length === 2
    ? [sector]
        .concat(_toConsumableArray(_lodash.default.intersection(_constants.SECTORS_NEIGHBORS[sector], sectors)))
        .sort()
    : [sector, _constants.SECTORS_NEIGHBORS[sector][sector % 2]].sort();
};
